...
describe('BackupService', () => {
  ...
  describe('importBackup', () => {
    ...
    it('rejects ZIP without manifest', async () => {
      ...
      await expect(service.importBackup(userId, zipBuffer)).rejects.toThrow('missing manifest.json')
    })
  ...
  describe('round-trip', () => {
    it('export then import preserves content', async () => {
      ...
      await env.DB.prepare(`UPDATE chapters SET r2_key = ? WHERE id = ?`).bind(r2Key1, ch1.id).run()
      await env.DB.prepare(`UPDATE chapters SET r2_key = ? WHERE id = ?`).bind(r2Key2, ch2.id).run()
